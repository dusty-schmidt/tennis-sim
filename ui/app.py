...
if action == 'Player Analytics':
	...
	st.write('Database Location:', config.DATABASE_PATH)
		from pipeline.processors.ownership import simulate_uniques_ownership; df = project_ownership(df); st.table(df[['player_name', 'salary', 'proj_mean', 'proj_ceil', 'p_win', 'proj_ownership']])
